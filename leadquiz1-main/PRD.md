...
Users complete 5 situational multiple-choice questions + 1 voice recording prompt. The system combines choice logic with multimodal speech analysis to generate a "New Grad Potential Report".
...
graph TD
    ...
    B --> C[Answer Q1-Q5 Situational MCQs]
    C --> D[Q6 Voice Prompt]
...
3    Q1-Q5 Multiple Choice    5 card-style situational questions
4    Q6 Voice Interaction    Mic permission → Recording → Multimodal feedback preview
...
4.2.3 Q1-Q5 Situational Questions (Optimized — North American Campus Context)

Each option maps to five persona dimensions: Act (Action), Comm (Communication), Inter (Interpersonal), Conf (Confidence), Strat (Strategy). Scores are averaged across Q1–Q5 then adjusted by voice analysis to produce a final 5D vector, which is matched to the 20-persona library via Euclidean distance (see Scoring Algorithm section).
...
| D | Forward the unfinished sections to everyone with a clear note on what's still missing. | 60 | 75 | 65 | 55 | 70 |
...
| D | Find someone else in line and strike up a conversation to make the wait useful. | 50 | 70 | 80 | 60 | 45 |
...
| D | Ask the recruiter what they're most curious about and tailor from there. | 45 | 80 | 75 | 65 | 75 |

---

**Q4. You're at a networking event and don't know anyone. You:**

| Option | Text | Act | Comm | Inter | Conf | Strat |
|--------|------|:---:|:----:|:-----:|:----:|:-----:|
| A | Find one person who looks equally lost and start there. | 40 | 45 | 60 | 35 | 30 |
| B | Introduce yourself to whoever's standing near the food or drinks. Low stakes. | 65 | 60 | 75 | 60 | 40 |
| C | Walk up to the most senior person in the room and introduce yourself directly. | 85 | 70 | 55 | 90 | 75 |
| D | Hang back and observe the room for a few minutes before making a move. | 20 | 30 | 35 | 25 | 55 |

---

**Q5. Your interviewer pushes back on something you just said: "I'm not sure I agree with that." You:**

| Option | Text | Act | Comm | Inter | Conf | Strat |
|--------|------|:---:|:----:|:-----:|:----:|:-----:|
| A | Hold your ground. "I hear you, but I still think..." | 70 | 65 | 45 | 90 | 55 |
| B | Ask them to elaborate before deciding whether to defend or adjust. | 45 | 60 | 65 | 55 | 85 |
| C | Say "that's a fair point" and incorporate their perspective into your answer. | 35 | 70 | 80 | 40 | 60 |
| D | Say "interesting, what's your take?" and flip it back to them. | 60 | 85 | 75 | 70 | 70 |

4.2.4 Q6 Multimodal Voice Prompt (Critical Feature)
Screen Copy:
...
**Button ID & GA4 Tracking:**
All primary and secondary CTA buttons must have a unique HTML `id` attribute. GA4 fires a `button_click` event on each interaction, passing the button `id` as a parameter.

| Button | HTML ID | GA4 Event | Location |
|--------|---------|-----------|----------|
| Find Out Now | `btn_hero_start` | `button_click` | Hero Section |
| Primary Share | `btn_share_primary` | `button_click` | Results Page |
| Would recruiters hire you? | `btn_unlock_secondary` | `button_click` | Results Page |
| See your readiness for 100+ roles | `btn_role_match_cta` | `button_click` | Unlock Page |
| Try a real interview | `btn_conversion_cta` | `button_click` | Unlock Page |
...
See section 4.2.3 for full per-option dimension scores across Q1–Q5.

User's raw vector = average of selected option scores across all five questions, per dimension.
...
**Fallback When Voice Data Unavailable:**
If voice analysis is unavailable for any reason (mic permission denied, user skips the voice prompt, or network timeout), all voice dimension adjustments default to **0** — no adjustment is applied. The MCQ vector proceeds directly to persona matching without modification. The result page renders normally. Voice-specific UI elements (WPM tag, Keywords tag, Confidence Score tag) are hidden from the results and unlock page when no voice data is present.
...
4. If tie (distance delta < 5) AND voice transcript is available, use keyword frequency from voice transcript as tiebreaker. If no voice transcript is available (mic denied, user skipped, or timeout), skip directly to step 5.
5. If still ambiguous (or no voice transcript), default to closest Conf score match.
...
button_click    Any tracked CTA pressed    button_id (see Section 4.2.7)
...
7.4 Result Page URL & Share Link Logic

The results page must render on a unique URL in the format `/result/{session_id}` to enable GA4 `page_view` tracking per session.

The share link distributed by users must always point to the quiz entry URL (e.g. `/` or `/start`), not the user's result URL. This ensures:

- New users entering via share links always start from the beginning of the quiz flow.
- GA4 can track share-driven acquisition separately via UTM parameters appended to the entry URL (e.g. `?utm_source=share&utm_medium=native_share&utm_campaign=aura_quiz`).
- Each user's result URL remains a non-shareable personal session; visiting it directly re-renders the quiz rather than showing another user's result.
